"""Compare roll=0 vs roll=90 at yaw=180 for flat board + handle direction."""
...
def run(label: str, roll: float, pitch: float) -> None:
    ...
    off.Normalize()
    print(f"{label}: layoutN=({ln[0]:+.2f},{ln[1]:+.2f},{ln[2]:+.2f}) handleOff=({off[0]:+.2f},{off[1]:+.2f},{off[2]:+.2f})")


for roll in (0, 90):
    run(f"roll={roll} pitch=0 yaw=180", roll, 0)
