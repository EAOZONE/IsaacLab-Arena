"""Find spawn pitch that points handle down (negative world Z) with yaw=180."""
...
OUT = open("/workspaces/isaaclab_arena/.lever_tmp/handle_sweep.log", "w", buffering=1)


def run(roll: float, pitch: float) -> None:
    ...
    ln = cache.GetLocalToWorldTransform(stage.GetPrimAtPath(LEVER + "/Layout_v9")).ExtractRotation().TransformDir(Gf.Vec3d(0, 0, 1))
    ln.Normalize()
    ...
    off.Normalize()
    stick = hxf.ExtractRotation().TransformDir(Gf.Vec3d(1, 0, 0))
    stick.Normalize()
    OUT.write(
        f"roll={roll:5.0f} pitch={pitch:5.0f} layoutN=({ln[0]:+.2f},{ln[1]:+.2f},{ln[2]:+.2f}) "
        f"off=({off[0]:+.2f},{off[1]:+.2f},{off[2]:+.2f}) stickX=({stick[0]:+.2f},{stick[1]:+.2f},{stick[2]:+.2f})\n"
    )
    OUT.flush()
