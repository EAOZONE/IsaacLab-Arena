"""Verify final spawn: roll=90 pitch=0 yaw=180."""
...
quat = quat_from_euler_xyz(torch.tensor([math.pi / 2.0]), torch.tensor([0.0]), torch.tensor([math.radians(yaw)]))[0].tolist()
...
ln.Normalize()
off = cache.GetLocalToWorldTransform(stage.GetPrimAtPath(HANDLE)).ExtractTranslation() - cache.GetLocalToWorldTransform(stage.GetPrimAtPath(BASE)).ExtractTranslation()
off.Normalize()
print(f"FINAL roll=90 yaw={yaw} layoutN=({ln[0]:+.2f},{ln[1]:+.2f},{ln[2]:+.2f}) handleOff=({off[0]:+.2f},{off[1]:+.2f},{off[2]:+.2f})")
