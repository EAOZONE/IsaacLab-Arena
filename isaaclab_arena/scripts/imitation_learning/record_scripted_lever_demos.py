...
if args_cli.object_name is None:
    args_cli.object_name = (
        Path(args_cli.usd).stem.lower().replace("(", "_").replace(")", "_")
    )
...
_BASE_LEVER_PUSH_LOCAL_OFFSET_OBJECTS = {
    "lever_again",
    "another_lever",
    "another_try_lever",
}
_BASE_LEVER_PUSH_LOCAL_OFFSET = [0.03603, 0.03933, 0.10620]
...
def _build_lever_path(env, device: torch.device):
    ...
    if (
        args_cli.object_name in _BASE_LEVER_PUSH_LOCAL_OFFSET_OBJECTS
        and push_local_offset_values == _DEFAULT_PUSH_LOCAL_OFFSET
    ):
        push_local_offset_values = _BASE_LEVER_PUSH_LOCAL_OFFSET
    ...
    world_up = torch.tensor(
        [0.0, 0.0, 1.0], device=device, dtype=wrist_contact_pos.dtype
    )
    ...
    joint_axis_local = torch.tensor(
        [_lever_joint_axis_local(args_cli.object_name)],
        device=device,
        dtype=handle_quat.dtype,
    )
...
_JOINT_AXIS_TOKEN_TO_LOCAL_VEC = {
    "X": [1.0, 0.0, 0.0],
    "Y": [0.0, 1.0, 0.0],
    "Z": [0.0, 0.0, 1.0],
}
...
def _lever_joint_axis_local(object_name: str) -> list[float]:
    ...
    assert (
        axis_token in _JOINT_AXIS_TOKEN_TO_LOCAL_VEC
...
def _settle_after_reset(env, n_steps: int = 5) -> None:
    ...
    action = torch.cat(
        [left_pos, left_quat, right_pos, right_quat, hold_hand]
    ).unsqueeze(0)
...
def main() -> None:
    ...
    with torch.inference_mode():
        while recorded < args_cli.num_demos and simulation_app.is_running():
            ...
            print(
                f"DBG settled handle pos={_dbg_pos.tolist()} quat={_dbg_quat.tolist()}",
                flush=True,
            )
