...
class AlexPutAndCloseDoorEnvironment(ExampleEnvironmentBase):
    """Sequential put-item-in-fridge-and-close-door task with the IHMC Alex robot.

    Alex variant of :class:`GR1PutAndCloseDoorEnvironment` — same LightWheel
    RoboCasa kitchen scene and two subtasks:
    1. Pick and place object onto the refrigerator shelf
    2. Close the refrigerator door
    The refrigerator starts open, the robot places the object inside, then closes it.

    Mount the ihmc-alex-sdk root so both alex-models (must include
    ``alex_V2_description``) and ihmc_hands_ros2 are visible inside the container::
    ...
    Usage::
        ...
        python isaaclab_arena/scripts/imitation_learning/record_demos.py \\
            ...
            --dataset_file /tmp/alex_put_and_close_door_demo.hdf5 \\
            --num_demos 1 --num_success_steps 2 \\
            alex_put_item_in_fridge_and_close_door \\
            --teleop_device openxr \\
            --embodiment alex_v2_ability_hands

    Stereo ZED X Mini cameras (``zed_left_cam``, ``zed_right_cam``) mount on
    ``HEAD_LINK``. Pass ``--enable_cameras`` to add camera observations and HDF5
    recordings (``camera_obs/zed_left_cam_rgb``, ``camera_obs/zed_right_cam_rgb``).
    """

    name = "alex_put_item_in_fridge_and_close_door"
    ...
    def get_env(self, args_cli: argparse.Namespace) -> IsaacLabArenaEnvironment:
        ...
        class PutAndCloseDoorTaskMimicEnvCfg(MimicEnvCfg):
            """
            Isaac Lab Mimic environment config class for Alex put and close door task.
            """
            ...
            def __post_init__(self):
                ...
                self.datagen_config.name = "alex_put_and_close_door_task_D0"
        ...
        # Set initial pose. Seeded from the GR1T2 standing pose in this kitchen;
        # Alex's base-to-hand kinematics and the ~0.15 m extra arm standoff differ,
        # so the z height and standoff likely need tuning in the viewer (run with
        # --viz kit and no teleop device) before recording demos.
    ...
    def add_cli_args(parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument("--embodiment", type=str, default="alex_v2_ability_hands", help="Robot embodiment to use")
