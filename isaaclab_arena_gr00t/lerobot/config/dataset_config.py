...
class Gr00tDatasetConfig:
    ...
    state_name_sim: str = field(
        ...
        metadata={"description": "Name of the state in the HDF5 file."},
    )
    left_eef_pos_name_sim: str = field(
        default=None,
        metadata={
            "description": "Name of the left eef position in the HDF5 file(optional)."
        },
    )
    left_eef_quat_name_sim: str = field(
        default=None,
        metadata={
            "description": "Name of the left eef quaternion in the HDF5 file(optional)."
        },
    )
    right_eef_pos_name_sim: str = field(
        default=None,
        metadata={
            "description": "Name of the right eef position in the HDF5 file(optional)."
        },
    )
    right_eef_quat_name_sim: str = field(
        default=None,
        metadata={
            "description": "Name of the right eef quaternion in the HDF5 file(optional)."
        },
    )
    teleop_base_height_command_name_sim: str = field(
        default=None,
        metadata={
            "description": "Name of the teleop base height command in the HDF5 file."
        },
    )
    teleop_navigate_command_name_sim: str = field(
        default=None,
        metadata={
            "description": "Name of the teleop navigate command in the HDF5 file."
        },
    )
    teleop_torso_orientation_rpy_command_name_sim: str = field(
        default=None,
        metadata={
            "description": "Name of the teleop waist roll pitch yaw command in the HDF5 file."
        },
    )
    action_name_sim: str = field(
        default="processed_actions",
        metadata={"description": "Name of the action in the HDF5 file."},
    )
    ...
    state_passthrough: bool = field(
        ...
        metadata={
            "description": (
                "When true, copy state_name_sim directly to the LeRobot state feature without joint remapping. "
                "Use for HDF5 streams already packed in the target schema."
            )
        },
    )
    action_passthrough: bool = field(
        default=False,
        metadata={
            "description": (
                "When true, copy action_name_sim directly to the LeRobot action feature without joint remapping. "
                "Use for HDF5 streams already packed in the target schema."
            )
        },
    )
    pov_cam_name_sim: str = field(
        default="robot_head_cam",
        metadata={"description": "Name of the POV camera in the HDF5 file."},
    )
    # Gr00t-LeRobot datafield
    state_name_lerobot: str = field(
        default="observation.state",
        metadata={"description": "Name of the state in the LeRobot file."},
    )
    action_name_lerobot: str = field(
        default="action",
        metadata={"description": "Name of the action in the LeRobot file."},
    )
    action_eef_name_sim: str = field(
        default="action.eef",
        metadata={"description": "Name of the eef action in the HDF5 file."},
    )

    video_name_lerobot: str = field(
        default="observation.images.ego_view",
        metadata={"description": "Name of the video in the LeRobot file."},
    )
    ...
    chunks_size: int = field(
        default=1000, metadata={"description": "Number of episodes per data chunk."}
    )
    # mp4 video
    fps: int = field(
        default=50, metadata={"description": "Frames per second for video recording."}
    )
    # Metadata files
    data_path: str = field(
    ...
    policy_joints_config_path: Path = field(
        ...
        metadata={
            "description": "Path to the YAML file specifying the joint ordering configuration used in dataset."
        },
    )
    robot_type: str = field(
        default="null",
        metadata={
            "description": "Type of robot embodiment used in the policy fine-tuning."
        },
    )
    ...
    task_index: int = field(
        default=0,
        metadata={
            "description": "Task index for the task description in LeRobot file."
        },
    )
    ...
    def __post_init__(self):
        ...
        self.lerobot_data_dir = (
            self.data_root / self.hdf5_name.replace(".hdf5", "") / "lerobot"
        )

        assert self.hdf5_file_path.exists(), f"{self.hdf5_file_path} does not exist"
        assert Path(
            self.policy_joints_config_path
        ).exists(), f"{self.policy_joints_config_path} does not exist"
        assert Path(
            self.action_joints_config_path
        ).exists(), f"{self.action_joints_config_path} does not exist"
        assert Path(
            self.state_joints_config_path
        ).exists(), f"{self.state_joints_config_path} does not exist"
        assert Path(
            self.info_template_path
        ).exists(), f"{self.info_template_path} does not exist"
        assert Path(
            self.modality_template_path
        ).exists(), f"{self.modality_template_path} does not exist"
        ...
        if self.lerobot_data_dir.exists():
            ...
            if (
                input(
                    f"Are you sure you want to remove {self.lerobot_data_dir}? (y/n): "
                )
                == "y"
            ):
        ...
        if self.teleop_base_height_command_name_sim:
            self.hdf5_keys["teleop_base_height_command"] = (
                self.teleop_base_height_command_name_sim
            )
        if self.teleop_navigate_command_name_sim:
            self.hdf5_keys["teleop_navigate_command"] = (
                self.teleop_navigate_command_name_sim
            )
        if self.teleop_torso_orientation_rpy_command_name_sim:
            self.hdf5_keys["teleop_torso_orientation_rpy_command"] = (
                self.teleop_torso_orientation_rpy_command_name_sim
            )
        ...
        if "teleop_base_height_command" in self.hdf5_keys:
            self.lerobot_keys["teleop_base_height_command"] = (
        ...
        if "teleop_torso_orientation_rpy_command" in self.hdf5_keys:
            self.lerobot_keys["teleop_torso_orientation_rpy_command"] = (
