...
Usage (host or training container; needs pandas, pyarrow, numpy, ffmpeg)::
    ...
    python isaaclab_arena_gr00t/lerobot/convert_lerobot_v3_to_gr00t.py \\
        --input_dir /datasets/lever_eef \\
        --output_dir /datasets/lever_eef_gr00t \\
        --modality_template isaaclab_arena_gr00t/embodiments/alex/alex_lever_eef_modality.json
"""
...
def load_v3_metadata(input_dir: Path) -> tuple[dict, pd.DataFrame, pd.DataFrame]:
    ...
    assert version.startswith(
        "v3"
    ), f"Expected a LeRobot v3.x dataset, got codebase_version={version!r}"

    episode_files = sorted((input_dir / "meta" / "episodes").glob("*/*.parquet"))
    assert (
        episode_files
    ), f"No episode metadata parquets under {input_dir / 'meta' / 'episodes'}"
...
def overwrite_action_from_state(frames: pd.DataFrame, dims: tuple[int, int]) -> None:
    ...
    assert (
        0 <= lo < hi <= min(states.shape[1], actions.shape[1])
...
def split_data_parquets(
    ...
    action_from_state_dims: tuple[int, int] | None,
) -> None:
    ...
    for ep_meta in episodes.itertuples():
        ...
        ep_frames = frames[frames["episode_index"] == ep_index].sort_values(
            "frame_index"
        )
        assert len(ep_frames) == int(
            ep_meta.length
        ), f"Episode {ep_index}: {len(ep_frames)} frames in data, {ep_meta.length} in metadata"
        out_path = output_dir / GR00T_DATA_PATH.format(
            episode_chunk=ep_index // CHUNKS_SIZE, episode_index=ep_index
        )
...
def split_videos(
    input_dir: Path, output_dir: Path, info: dict, episodes: pd.DataFrame
) -> None:
    video_keys = [
    ...
    for video_key in video_keys:
        for _, ep in episodes.iterrows():
            ...
            assert (
                abs(n_frames - expected) <= 1
            ), f"{dst}: cut {n_frames} frames, episode length is {expected}"
...
def write_meta(
    ...
    with open(meta_dir / "tasks.jsonl", "w") as f:
        for task, row in tasks.iterrows():
            f.write(
                json.dumps({"task_index": int(row["task_index"]), "task": task}) + "\n"
            )
...
def convert(
    ...
    assert (
        modality_template.is_file()
    ), f"Modality template not found: {modality_template}"
    ...
    if action_from_state_dims is not None:
        print(
            f"Overwriting action dims [{action_from_state_dims[0]}:{action_from_state_dims[1]}] from state"
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--input_dir", type=Path, required=True, help="LeRobot v3.0 dataset root."
    )
    parser.add_argument(
        "--output_dir",
        type=Path,
        required=True,
        help="Output root for the GR00T-layout dataset.",
    )
    parser.add_argument(
        "--modality_template",
        type=Path,
    ...
    convert(
        args.input_dir,
        args.output_dir,
        args.modality_template,
        args.action_from_state_dims,
    )
